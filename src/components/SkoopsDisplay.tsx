...
import SkoopsPopup from "./SkoopsPopup";
...
const SkoopsDisplay = ({ skoops, level, compact = false, className = "" }: SkoopsDisplayProps) => {
  return (
    <SkoopsPopup skoops={skoops} level={level} variant="customer">
      <motion.div 
        className={`flex items-center ${compact ? "gap-1" : "gap-2"} ${className} cursor-pointer hover:opacity-90`}
